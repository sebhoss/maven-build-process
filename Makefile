...
update-properties: ##@maintenance Update all properties to their latest versions
	...
	git add **/**/pom.xml
	git commit -a -s -S -m 'Update properties to latest version'
...
.PHONY: docker-verify
docker-verify: create-build-environment ##@docker Verify project in pre-defined build environment
	docker-compose \
	   -f build/docker/build-environment.yml \
	   run --rm verify-project
